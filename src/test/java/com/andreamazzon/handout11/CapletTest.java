...
import com.andreamazzon.handout11.LIBORMarketModelConstructionWithDynamicsAndMeasureSpecification.Dynamics;
import com.andreamazzon.handout11.LIBORMarketModelConstructionWithDynamicsAndMeasureSpecification.Measure;
 ...
 * measure (spot or terminal) and dynamics (normal or lognormal). In particular, we focus on caplet prices.
 *
...
public class CapletTest {
	 ...
	 */
	@Test
	public void testWithPrices() throws CalculationException, InterruptedException {

		String dynamics;//it will be used just for the plots

		//we first consider log-normal dynamics, and then normal dynamics
		for(final Dynamics typeOfDynamics : new Dynamics[] { Dynamics.LOGNORMAL, Dynamics.NORMAL}) {
			...
			DoubleUnaryOperator capletFunction = (strike) -> {
				...
				try {
					return notional * caplet.getValue(lmm);
			...
			myPlot.setTitle("Caplet price using " + dynamics + " dynamics");
