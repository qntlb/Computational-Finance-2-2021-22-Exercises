...
public class LIBORMarketModelConstructionWithDynamicsAndMeasureSpecification {
	 ...
	 * @return an object implementing LIBORModelMonteCarloSimulationModel, i.e.,
	...
	public static final TermStructureMonteCarloSimulationModel createLIBORMarketModel(int numberOfPaths,
		...
		/*
		 * Step 7 Here you substitute your covariance model with a new one built on top
		 * of it. The new model is given by (a L0 + (1-a)L) F where
		 * a=parameterForBlended is the displacement parameter, L is the component of
		 * the stochastic process, L_0 is its value at time zero and F is the factor
		 * loading from the given covariance model. Here the point is
		 * parameterForBlended: we construct it is such a way that is 0 if the dynamics
		 * are log-normal (so in this case nothing happens) and 1 if they are normal (in
		...
		/*
		 * Empty array of calibration items and a RandomVariableFactory, to be given to
		 * the constructor of LIBORMarketModelFromCovarianceModel
		 */
